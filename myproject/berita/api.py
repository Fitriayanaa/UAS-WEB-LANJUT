...
def api_kategori_detail(request, id_kategori):
    try:
        ...
        return Response({'masage:data kategori ditemukan'}, status=status.HTTP_404_NOT_FOUND)
...
def api_artikel_list(request):
    key_token = "fitriayanaa"

    token = request.headers.get('token')
    if token == None:
        return Response({'massage: masukan token'}, status=status.HTTP_401_UNAUTHORIZED)
    
    if token !=  key_token:
        # token = Token.objects.filter(token=token)
        return Response({'massage: masukan token yang benar'}, status=status.HTTP_401_UNAUTHORIZED)
...
def api_artikel_detail(request, id_artikel):
    try:
        ...
        return Response({'masage:data artikel ditemukan'}, status=status.HTTP_404_NOT_FOUND)
